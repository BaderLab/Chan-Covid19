...
faFile<- "/home/project_resources/data/mafft/200420_genomes/NCBI_completeSeq_200420_passQC_probes_aligned.fa"
...
p <- p + ggtitle("NCBI 20 April 2020 - Ns")
p_orig <- p

# annotate ORF1a probes
p <- p + annotate("rect",xmin=22260, xmax=22400, ymin=0, ymax=0.5,alpha=0.3,fill="red",colour=NA)
p <- p + annotate("text",x=22900,y=0.6,label="ORF1a",size=5)
# annotate RDRP probes
p <- p + annotate("rect",xmin=15420, xmax=15540, ymin=0, ymax=0.5,alpha=0.3,fill="red",colour=NA)
p <- p + annotate("text",x=15700,y=0.6,label="RdRp",size=5)
# annotate E gene probes
p <- p + annotate("rect",xmin=26260, xmax=26400, ymin=0, ymax=0.5,alpha=0.3,fill="red",colour=NA)
p <- p + annotate("text",x=26900,y=0.6,label="E gene",size=5)

pdf("NCBI_countN.pdf",width=14,height=6); 
tryCatch({
	print(p); 
	p <- p_orig + ylim(c(0,2.5))
# annotate ORF1a probes
p <- p + annotate("rect",xmin=22260, xmax=22400, ymin=0, ymax=2,alpha=0.3,fill="red",colour=NA)
p <- p + annotate("text",x=22900,y=2.1,label="ORF1a",size=5)
# annotate RDRP probes
p <- p + annotate("rect",xmin=15420, xmax=15540, ymin=0, ymax=2,alpha=0.3,fill="red",colour=NA)
p <- p + annotate("text",x=15700,y=2.1,label="RdRp",size=5)
# annotate E gene probes
p <- p + annotate("rect",xmin=26260, xmax=26400, ymin=0, ymax=2,alpha=0.3,fill="red",colour=NA)
